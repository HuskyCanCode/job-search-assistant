...
For completed job searches, deliver a local HTML report using [assets/report-template.html](../assets/report-template.html) through the helper below, plus a Markdown companion. This is the default skill workflow; the user need not request HTML separately. Follow explicit format preferences, and do not require saved reports for role-only or keyword-only requests. When local generation is unavailable, provide chat tables and state that no HTML file was created. Use the actual search evidence, never the fictional sample's records.
...
Provide evidence details for the strongest opportunities and any recommendation that hinges on a disputed or missing requirement. Keep permitted inaccessible leads, known hard blockers, and closed postings from active sources separate from active recommendations. Show a short search log with queries/sources used, check date, coverage, and why the search stopped. Excluded routes use `skipped`, while actual permitted requests that fail use `blocked`. Independent host discovery is `web_search`; do not imply that it searched an excluded board. Sources excluded by [job-sources.md](job-sources.md) must not be fetched, targeted through another engine, ingested or recommended as manual alternatives. Their names and policy links belong only in exclusion/skipped explanations and the audit. Ignore incidental excluded or restricted snippets rather than quoting, scoring or reconstructing them, and do not ingest Lensa content into AI outputs. User-pasted copies or a newly available integration do not reactivate an excluded source.

Prefer independently verified employer facts and direct employer job links from active sources where compatible with their terms. Provider terms may restrict content use, deep links, retaining snippets or raw responses: preserve only allowed attribution and note resulting evidence limits. Excluded sources such as Glassdoor, Dice and Welcome to the Jungle receive no job links or homepage/manual-query recommendations. Do not evade restrictions by copying responses into this schema or a checkpoint. Preserve an active permitted feed's mandatory source credit, original job links and application routes even when a different employer URL exists. If zero active matches remain, report zero and suggest specific changes to soft preferences; do not fabricate replacements. If the user requested only an excluded source, explain that no search was performed rather than silently replacing it with other sources.

## Repeatable helper

Run from the skill repository root for the default HTML and Markdown reports:

```sh
python3 scripts/build_report.py private/search-input.json --output private/job-report.md --html private/job-report.html
```

`--output` is required and writes Markdown (`--markdown` is an alias). Include `--html` by default when running the skill: it renders the bundled template with the same validated evidence and derived scores. The CLI flag remains optional for other export workflows and supports version 2 company reports and version 1 job-only inputs. Add `--csv private/job-report.csv` for an optional job summary, with company fields appended in version 2. Add `--companies-csv private/company-report.csv` for an optional separate company summary; it requires version 2. The helper uses Python 3.10+ and the standard library. It validates input, computes the rubric and renders tables without network requests. The agent must verify facts and classify evidence first. It cannot determine whether evidence is truthful or a job is actually open.
...
For the default delivery, verify the generated files, present the HTML link first in the final reply with result counts and material limitations, and link the Markdown companion for full tables and keywords. Avoid duplicating the entire report in chat unless requested. Open the HTML locally when supported; creating it does not publish or upload it. Only claim a file was saved or opened after successful tool confirmation. The [fictional HTML sample](../examples/sample-report.html) demonstrates the layout, not verified vacancies or completion of the 20-company target. GitHub displays its source/download view; download the file to view the rendered report. See [advanced export guidance](../docs/advanced.md#try-the-report-generator) for browser behavior and [preview help](../docs/report-preview.md).
...
Each discovery source has a nonempty `source` name and an absolute HTTP(S) `url`. Keep the employer's preferred original posting as the job `url` only where compatible with applicable source terms, and allowed active-board or search-result links in `discovered_via`. Never replace a required feed job link or application route without preserving the obligation. This is provenance, not proof of verification or permission. If a job cannot be represented with permitted content and its required direct link, omit that job record. Do not use a homepage as a vacancy or recommend an excluded board as a manual alternative. The CSV appends `discovery_sources` and `discovery_urls` columns; the source coverage ledger is in Markdown. Assess each output's attribution requirements separately.
...
| url | Permitted observed HTTP(S) search URL, active source entry point, or public API request URL; for an excluded source's skipped explanation, use its audited policy URL rather than a job/homepage recommendation |
