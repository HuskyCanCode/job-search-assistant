...
- Present a linked company name and company summary, followed by **up to three recommended open jobs** at that employer. Use permitted links to verified original postings and preserve any licensed feed's required source credit, job links and application routes. Provide an application entry link when separately available and compatible with those conditions.
...
Verify employer identity with each job, but delay employee size, workforce trend and layoff research until the employer has at least one viable verified open vacancy: a complete permitted description, compatible duties/level/location and no confirmed unmet hard constraint. Keep any material eligibility unknowns visible. A raw collected lead or local triage label does not satisfy this condition. Research each qualifying employer once and share its permitted facts across jobs; do not spend the initial discovery budget researching employers with no viable vacancy.

The first-five interim table may show size and trend as **Unknown — research pending**, and layoffs as **Unknown — research pending**. This is neither zero employees nor “no layoff found.” The final report still includes every requested field, researched after qualification or marked with a specific evidence/access limitation. A narrow, useful pass through official sources and the required layoff searches is enough when further queries repeat the same evidence; revisit only credible contradictions or new primary evidence. Do not leave a field pending merely because five companies have already been shown.
...
4. Clearly labeled third-party estimates or company profiles obtained through a permitted method, with applicable use and storage rights, when better evidence is unavailable. Do not fall back to restricted source retrieval or targeted indexed collection.

For each fact retain a permitted direct source URL and required attribution, source publication date when known, the fact's **as-of date**, actual check date, entity/geographic scope, and whether the value is reported, estimated or derived. If a source's conditions cannot be satisfied by the report, use another source or mark the fact unknown rather than dropping required credit or links. A current page can contain an old fact. A crawl date is neither its publication date nor its measurement date.

A private employer registry can reuse observed identity/board metadata and its permitted policy evidence across runs. Follow the minimal schema in [search-state.md](search-state.md), including provenance, policy scope/check date and retention basis; keep applicant/resume information, workforce-research copies and raw descriptions out of that registry. Recheck that the employer relationship, route, user scope and permission are still current before using a stored board. Permission records do not grant permission, and a stored board or old job check does not establish a current vacancy. Do not automatically extend a permitted per-run copy into permanent storage.
...
For every company entry, provide its permitted official link, rating and contributing count, size with date/source, headcount trend with period/source, and latest publicly reported layoff result with date type/scope/source. Follow it with up to three linked job rows containing location/work style, pay, employment type, fit/coverage, strengths, gaps, eligibility and a useful next action. Preserve source-specific attribution and application/link requirements in every delivered format. If the helper cannot preserve them, use a suitable manual rendering or omit incompatible material. Keep missing facts visible as unknown, and report fewer companies or jobs when the evidence does not support the target.
