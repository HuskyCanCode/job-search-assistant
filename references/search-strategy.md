...
2. **Discover in parallel.** Check permitted access, use and storage for each provider/method once, reuse that decision, and batch queries within provider and host limits. Start with independent employer discovery and supported public boards whose relevant rights are established. The catalog is not a required query checklist: restricted or unverified boards remain manual by default, including targeted collection through another search engine. User-named sources get an allowed attempt only when the scope is established; otherwise record a skipped/manual-route explanation and continue. Spread role variants across permitted routes rather than every keyword across every board. Different queries on one source are not coverage of other sites.
...
Begin with close roles. Choose permitted routes using [job-sources.md](job-sources.md) and [source-policy-audit.md](source-policy-audit.md), prioritizing independent employer discovery and employer pages. Niche, local, remote and entry-level sources are eligible only within established scope. Do not query every catalog site or target a manual-only source because the user named it. Respect narrower source choices and exclusions. Follow observed permitted employer career and applicant-tracking-system (ATS) links; never guess an employer's ATS identifier or a posting URL.

Execute independent role/employer queries in small parallel batches through the supported host search integration within its scope. Inspect and use only permitted result data. Use direct board APIs or automated browser access only when the source policy permits the actual method and downstream report use. Incidental restricted snippets are not evidence: do not quote, score, copy or reconstruct them, including extracting details to recreate the listing elsewhere. Ignore them and continue independent employer discovery from the user's goals and permitted evidence. Do not ingest Lensa content into AI outputs. A user-pasted copy is not a workaround for these restrictions. For a source with no allowed route, provide only permitted homepage guidance and generic manual search terms.
...
Record each actual source/query attempt in `search_sources` using the contract in [report-format.md](report-format.md). Capture `source`, permitted observed `url`, `method`, exact `query`, `status`, `checked_at`, `results_seen`, `verified_open`, and `notes`, subject to access and retention terms. Methods are `platform_search`, `web_search`, `employer_site`, and `public_api`. A homepage read without a role query is source setup, not a completed search. Record policy-disabled access as `skipped` with its reason, not `blocked` unless an actual permitted request failed. An independent host web search is a `web_search` row; do not count incidental restricted snippets as a board search or verified job. Preserve only permitted links: where deep linking is restricted, use allowed homepage guidance outside job records.
...
Inspect the specific original vacancy through a permitted employer page or employer-linked ATS route. Finding a public URL, using a familiar browser, or receiving a search snippet does not authorize restricted destination access or report reuse. A snippet, job-search landing page or generic careers page is not a verified vacancy. Do not bypass login walls, CAPTCHAs or automation restrictions. If no permitted route confirms the full description and current application availability, retain an explicitly unverified/manual-review lead only if its content and links may be used. Glassdoor, Dice and US Welcome to the Jungle default to permitted homepage/manual-query guidance, not deep-linked job records. Continue with another employer.
...
Only score jobs whose permitted descriptions provide enough evidence for comparison. Keep permitted but blocked, snippet-only or otherwise unverified leads separate with the reason and an allowed source link, unscored. Restricted snippets are ignored, not retained as leads. Use homepage/manual-query guidance where job content or deep links cannot be retained. Exclude confirmed closed jobs from current recommendations or show them separately as closed examples when their reuse is allowed. If dates, pay, eligibility, or application status remain unknown, say so in the table.
