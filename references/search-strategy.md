...
2. **Discover in parallel.** For a broad search, make an initial role/location query on each relevant site in the default catalog, grouped into small batches within the host tool's limits. Spread role variants across sources rather than executing every keyword on every board. Explicitly requested sites get an attempt even when other sources already returned enough results; record exclusions, market mismatches and a user-imposed time limit as skipped. Different queries on one source are not coverage of other sites.
...
5. **Bound access recovery.** Prefer readable originals or documented public board APIs. For an inaccessible posting, try one accessible employer route or indexed fallback; use an ordinary browser check when it can resolve a specific ambiguity, such as JavaScript rendering or a contradictory status. Stop at sign-in/CAPTCHA blocks. Do not repeatedly open the same failing URL, recreate browser tabs, or repeat unchanged empty queries. Retry only when new evidence or a changed access condition makes it useful. Record the limit and move to another candidate.
6. **Reuse evidence within the run.** Share completed job checks, employer facts, failed routes and exclusion reasons. Do not re-fetch an unchanged description or re-read the resume merely because the task moved to another worker. Check a job again when status conflicts, the source changes, or a later run needs fresh verification. Saved historical facts may be reused with their real dates and sources, but cached listings are never current verification for a later run.
