...
Read [references/job-sources.md](references/job-sources.md). The default broad-search catalog includes LinkedIn, Indeed, ZipRecruiter, Built In, Y Combinator Jobs, Wellfound, HiringCafe, SimplyHired, Lensa, and Teal, alongside original employer pages. Use its source-specific routes and role/market guidance; honor user-named sources and exclusions. The user does not need to enumerate boards. Attempt relevant catalog sources in small parallel batches; explain skipped or inaccessible sources in the coverage ledger.
